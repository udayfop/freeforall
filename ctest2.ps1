...
function Check-DiskSpace {
    ...
    try {
        ...
        if ($drive) {
            ...
            if ($free -lt 15) {
                Add-Content -Path E:\Harshitha\cdrivespace\Output.csv -Value "$ComputerName,$used,$free,$total,Low Disk Space"
            } else {
                Add-Content -Path E:\Harshitha\cdrivespace\Output.csv -Value "$ComputerName,$used,$free,$total,Healthy"
            }
        } else {
            Add-Content -Path E:\Harshitha\cdrivespace\Output.csv -Value "$ComputerName,,, ,Drive Not Found"
        }
    }
    catch {
        Add-Content -Path E:\Harshitha\cdrivespace\Output.csv -Value "$ComputerName,,, ,Error: $($_.Exception.Message)"
    }
}
